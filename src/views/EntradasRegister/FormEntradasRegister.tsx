...
//   // Llamar items de la base de datos desde la ruta http://localhost:3000/api/v1/items GET
...
const FormEntradasRegister: React.FC<FormEntradasRegisterProps> = ({
  ...
  const toggleCreatedAtField = () => {
    setShowCreatedAtField((prev) => !prev);
    if (!showCreatedAtField) {
      formData.createdAt = ""; // Inicializa el campo si se habilita
    } else {
      delete formData.createdAt; // Elimina el campo si se deshabilita
    }
  };
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      >
        ...
        {/* Botón para agregar el campo de fecha de registro */}
        {/* <div className="col-span-2 flex items-center justify-between">
          ...
          <button
            type="button"
            className={`${
              showCreatedAtField ? "bg-red-500" : "bg-blue-500"
            } text-white px-4 py-2 rounded`}
            onClick={toggleCreatedAtField}
          >
            {showCreatedAtField ? "Quitar Fecha" : "Agregar Fecha"}
          </button>
        </div> */}

        {/* Campo de fecha visible condicionalmente */}
        {showCreatedAtField && (
          <Input
            id="createdAt"
