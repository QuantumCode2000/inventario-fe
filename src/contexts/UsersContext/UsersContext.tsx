...
const UsersProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get(
        "http://http://54.221.108.114/:3000/api/v1/usuarios",
      );
  ...
  const removeUser = async (ci: string) => {
    try {
      ...
      await axios.delete(
        `http://http://54.221.108.114/:3000/api/v1/usuarios/${userToRemove.id}`,
      );
  ...
  const updateUser = async (updatedUser: Partial<User>) => {
    ...
    try {
      const response = await axios.patch(
        `http://http://54.221.108.114/:3000/api/v1/usuarios/${id}`,
        rest,
      );
      setUsers((prevUsers) =>
